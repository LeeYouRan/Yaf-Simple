                ...
                case 'oracle':
                    $attr = [
                        ...
                        'dbname' => $options['server']
                            ?
                            '//' . $options['server'] . ($is_port ? ':' . $port : ':1521') . '/' .
                ...
                if (isset($match['operator'])) {
                    ...
                    if (in_array($operator, ['>', '>=', '<', '<='])) {
                        ...
                        foreach ($value as $index => $item) {
                            ...
                            $like_clauses[]               =
                                $column . ($operator === '!~' ? ' NOT' : '') . ' LIKE ' . $map_key . 'L' . $index;
                        ...
                        if ($type === 'array') {
                            ...
                            $data_type =
                                (is_numeric($value[0]) && is_numeric($value[1])) ? PDO::PARAM_INT : PDO::PARAM_STR;
        ...
        foreach ($columns as $column => $replacements) {
            if (is_array($replacements)) {
                foreach ($replacements as $old => $new) {
                    ...
                    $stack[] =
                        $this->columnQuote($column) . ' = REPLACE(' . $this->columnQuote($column) . ', ' . $map_key .
                        'a, ' . $map_key . 'b)';
